...
class TwentyCRMServer {
      ...
      return {
        tools: [
          ...
          // Workspace Members
          {
            ...
            description: "List workspace members (human users and bot accounts, e.g. the 'Claude' assignee) with their IDs and emails",
            inputSchema: {
              type: "object",
              properties: {
          ...
          // Claude-assigned Tasks
          {
            ...
            description: "Create a task and assign it directly to the Claude bot workspace member (CLAUDE_ASSIGNEE_ID). Use this instead of create_task whenever a task should be worked on by Claude rather than a human.",
            inputSchema: {
              type: "object",
              properties: {
                title: { type: "string", description: "Task title" },
                body: { type: "string", description: "Task description / instructions for Claude" },
                dueAt: { type: "string", description: "Due date (ISO 8601 format)" },
                status: { type: "string", description: "Task status", enum: ["TODO", "IN_PROGRESS", "DONE"] },
                position: { type: "number", description: "Position for ordering" }
              },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          ...
          // Claude-assigned task operations
          case "create_claude_task":
            return await this.createClaudeTask(args);
  ...
  async listWorkspaceMembers(params = {}) {
    ...
    const endpoint = `/rest/workspaceMembers?limit=${limit}&offset=${offset}`;
    const result = await this.makeRequest(endpoint);
    return {
      content: [
        {
          type: "text",
          text: `Workspace members: ${JSON.stringify(result, null, 2)}`
        }
      ]
    };
  }

  // Claude-assigned task methods
  async createClaudeTask(data) {
    const claudeAssigneeId = process.env.CLAUDE_ASSIGNEE_ID;
    if (!claudeAssigneeId) {
      throw new Error(
        "CLAUDE_ASSIGNEE_ID environment variable is not set. Invite a 'Claude' workspace member in Twenty, " +
        "look up its ID with list_workspace_members, and set CLAUDE_ASSIGNEE_ID in the server's .env file."
      );
    }
    const taskData = { ...data, assigneeId: claudeAssigneeId };
    const result = await this.makeRequest("/rest/tasks", "POST", taskData);
    return {
      content: [
        {
          type: "text",
          text: `Created task assigned to Claude: ${JSON.stringify(result, null, 2)}`
        }
      ]
    };
  }
